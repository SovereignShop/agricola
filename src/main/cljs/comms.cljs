(ns agricola.comms
  ...
  (:require
   ...
   [agricola.events.server :as server-events]
   [agricola.comms-common :refer [TransitPacker]]
...
(defmethod server-event ::facts
  ...
  (re-posh/dispatch [:agricola.events.facts/add-facts (with-meta datoms tx-meta) true]))

(defmethod server-event :agricola.comms/started-processing
  [_]
  (re-posh/dispatch [::server-events/set-server-status [:swig/ident idents/server-events] true]))

(defmethod server-event :agricola.comms/finished-processing
  [_]
