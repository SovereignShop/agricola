...
(def-event-ds ::new-game [db _]
  (let [session     (d/entity db [:swig/ident idents/session])
        ...
        first-color (:agricola.team/color first-team)]
    (into [{:game/finished?     false
           ...
           {:agricola.round/number           1
            :agricola.round/current-turn     -5
            :agricola.round/blue-cards-count (case first-color :blue 9 8)
            :agricola.round/red-cards-count  (case first-color :red 9 8)
            :agricola.round/current-team     (:db/id first-team)
            :db/id                            -4}
           {:db/id                     -5
            :agricola.turn/team       (:db/id first-team)
            :agricola.turn/word       ""
...
(def-event-ds ::new-round [db [_ game-id]]
  (let [game         (d/entity db game-id)
        ...
        round-number (apply max (map :agricola.round/number rounds))]
    (into [{:agricola.round/number           (inc round-number)
            :agricola.round/turns            -2
            :agricola.round/current-turn     -2
            :agricola.round/blue-cards-count (case first-color :blue 9 8)
            :agricola.round/red-cards-count  (case first-color :red 9 8)
            :agricola.round/current-team     (:db/id first-team)
            :db/id                            -1}
...
(def-event-ds ::join-team [db [_ color game-id]]
  (let [session (d/entity db [:swig/ident idents/session])
    ...
    (if (and tid pid team-id (not= tid team-id))
      [[:db/retract tid :agricola.team/players pid]
       [:db/add team-id :agricola.team/players pid]]
      (if (and team-id (not= tid team-id))
        [(assoc (utils/make-player user-id :guesser) :db/id -1)
         [:db/add team-id :agricola.team/players -1]]
        (do (info "No update")
            [])))))
...
(def-event-ds ::choose-player-type [db [_ game-id player-type]]
  (let [session (d/entity db [:swig/ident idents/session])
        player  (d/q '[:find ?pid .
                     ...
                     game-id)]
    [[:db/add player :agricola.player/type player-type]]))

(def-event-ds ::randomize-teams [db _]
  (let [session (d/entity db idents/session)
        game    (d/entity db (:session/game session))
        teams   (:game/teams game)
        players (d/pull-many db [:agricola.team/players] teams)
        [a b]   (split-at (/ (count players) 2) (shuffle (apply concat players))) ]
    (concat
     (for [tid teams]
       [:db.fn/retractAttribute tid :agricola.team/players])
     [[:db/add (first teams) :agricola.team/players a]]
     [[:db/add (first teams) :agricola.team/players b]])))
