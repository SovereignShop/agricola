...
(defn do-login [credentials]
  (re-posh/dispatch [:agricola.events.app-state/login-waiting])
  (request
   {:uri     (gstr/format "%slogin" js/window.location.href)
    ...
    :params  credentials
    :success (fn [resp]
               (re-posh/dispatch [:agricola.events.app-state/login-success])
               (let [datoms (into [[:db/retractEntity [:swig/ident :swig/root-view]]
                 ...
                 (re-posh/dispatch [:agricola.events.facts/add-facts datoms true])))
    :error   (fn [resp]
               (re-posh/dispatch [:agricola.events.app-state/login-fail])
               (warn "Login failed"))}))
