(ns agricola.views.game
  (:require
   ...
   [agricola.subs.stats :as stat-subs]
...
(defn popover! [content label title]
  (re-posh/dispatch [:agricola.events.popover/show
...
(defn display-card
  [game-id
   {:keys [:agricola.word-card/word
           :agricola.word-card/position
  ...
  (let [character-card-id (:db/id character-card)
        ...
        played?           (:agricola.character-card/played? card)
        role              (:agricola.character-card/role card)
...
(defn board-info [tab-id game-id cards]
  (let [current-team                 @(re-posh/subscribe [::game-subs/current-team game-id])
        team-color                   (:agricola.team/color current-team)
...
(defn board-grid [game-id cards]
  (let [cards (->> cards
                   (sort-by :agricola.word-card/position)
...
(defn turn-handler [game-id]
  (let [turn        @(re-posh/subscribe [::game-subs/current-turn game-id])
        ...
        player-type @(re-posh/subscribe [::game-subs/player-type game-id])
        codemaster? (= player-type :codemaster)
        submitted?  (:agricola.turn/submitted? turn)]
    (if (and codemaster? (not submitted?))
      [h-box
       :class "center"
       :children
       [[input-text
         :model (str (:agricola.turn/word turn))
         :placeholder "word"
         :on-change #(re-posh/dispatch [::game-events/set-word turn-id %])]
        [input-text
         :model (str (:agricola.turn/number turn))
         :placeholder "number"
